...
"""
adapted from https://github.com/sallamander/horizon-detection. Thank you!
"""
def detect_horizon_line(image_grayscaled):
    ...
    if(len(np.where(image_closed[:, horizon_x1] == 0)[0]) == 0):
         horizon_y1 = -1
         horizon_x1 = -1
    else:
        horizon_y1 = max(np.where(image_closed[:, horizon_x1] == 0)[0])
    if(len(np.where(image_closed[:, horizon_x2] == 0)[0]) == 0):
         horizon_y2 = -1
         horizon_x2 = -1
    else:
